...
export default function ProductForm({ onSave, initialData, mode = "add" }) {
  ...
  const [quantity, setQuantity] = useState(data.stockQuantity || 0);
  ...
  const handleSubmit = (e) => {
    ...
    if (!selectedCategory) {
      alert("⚠️ Vui lòng chọn danh mục hợp lệ.");
      return;
    }
      ...
      <div>
        ...
        <div className="flex gap-2 mt-2 flex-wrap">
          {images.map((img, idx) => {
            ...
            const src = isBase64 ? img : `http://localhost:8080/storage/product-${data.id}/${img}`;
            return (
